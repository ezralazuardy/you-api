import { ZodiacDto } from './dtos/zodiac.dto';
import { Moment } from 'moment';
import moment from 'moment';

/**
 * Get the user's zodiac based on their birthday.
 ...
 * @returns {string} The user's zodiac.
 */
export function getZodiac(birthday: Moment): ZodiacDto {
  // formatting birthday to get a date as YYYYMMDD
  const date = birthday.format('YYYYMMDD');

  // initialize the year
  let year: Moment | null = null;
  ...
  for (let yearIndex = 0; yearIndex < signs.length; yearIndex++) {
    if (date >= signs[yearIndex][0]) {
      year = moment(signs[yearIndex][0], 'YYYYMMDD');
      sign = signs[yearIndex][1];
      element = signs[yearIndex][2];
      break;
    }
  }

  // get the description index of the sign
  const signDescriptionIndex = signDescriptions.flatMap((sub, idx) =>
    sub.includes(sign) ? idx : [],
  )[0];

  // get the description index of the element
  const elementDescriptionIndex = elementDescriptions.flatMap((sub, idx) =>
    sub.includes(element) ? [idx] : [],
  )[0];

  return {
    sign: {
      name: sign,
      description: signDescriptions[signDescriptionIndex][1],
      pros: signDescriptions[signDescriptionIndex][2],
      cons: signDescriptions[signDescriptionIndex][3],
    },
    element: {
      name: element,
      description: elementDescriptions[elementDescriptionIndex][1],
    },
    chineseYear: !year
      ? null
      : {
          timestamp: year.unix(),
          formatted: year.format('YYYY-MM-DD'),
        },
  };
}

// collection to match first day of the chinese year, which sign it is, and its element
const signs = [
  ['20990121', 'goat', 'earth'],
...
const signDescriptions = [
  [
    'rat',
    'People born in the Year of Rat are instinctive, acute and alert in nature which makes them to be brilliant businessmen. They can always react properly before the worst circumstances take place. They are also sophisticated and popular in social interaction. They are sanguine and very adaptable, being popular with others.',
  ...
  [
    'dog',
    'They are usually independent, sincere, loyal and decisive according to Chinese zodiac analysis. They are not afraid of difficulties in daily life. These shining characteristics make them have harmonious relationship with people around.',
    'Valiant, loyal, responsible, clever, courageous, lively',
    'Sensitive, conservative, stubborn, emotional',
  ],
  [
    'pig',
    "People with Chinese zodiac sign Pig are considerate, responsible, independent and optimistic. They always show generousness and mercy to endure other people's mistakes, which help them gain harmonious interpersonal relationship. However, sometimes they would behave lazy and lack actions. In addition, pure hearts would let them be cheated easily in daily life.",
